...
it.only('GET /v1/products/by-material/:materialId', async () => {
  const response = await request(server.express).get(`/v1/products/by-material/${1}`); // Envía una petición GET a la ruta '/v1/products/by-material/1' para obtener productos según su material (materialId = 1)
