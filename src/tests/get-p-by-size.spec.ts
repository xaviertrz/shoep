...
it.only('GET /v1/products/by-size/:sizeId', async () => {
  const response = await request(server.express).get(`/v1/products/by-size/${1}`); // Envía una petición GET a la ruta '/v1/products/by-size/1' para obtener productos según su tamaño (sizeId = 1)
