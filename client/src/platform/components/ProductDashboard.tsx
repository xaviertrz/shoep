          ...
          {filteredProducts.length > 0 ? (
            filteredProducts.map(product => (
              <tr key={product.id} className={`flex text-left items-center border-gray-200 bg-gray-50 border-b`}>
                ...
                <td className="px-4 flex flex-col w-full py-2">
                ...
                {product.blocked && (
                  <td className="flex flex-col rounded-md px-4 py-1 w-full bg-red-600">
                    <span className="text-white uppercase font-medium text-sm">Bloqueado</span>
                  </td>
                )}
                <td className="px-4 py-2 justify-end w-full flex">
                  <Link
                    to={product.blocked ? `` : `/editar-producto/${product.uuid}`}
                    className={`flex items-center gap-2 px-3 rounded-md py-1 w-fit text-black font-medium border  transition duration-300 ease-in-out ${product.blocked ? 'hover:cursor-not-allowed bg-gray-200' : 'bg-white border-gray-200 hover:bg-gray-200'}`}
                  >
                    <FaCog size={20} />
                    Modificar
                  </Link>
            ...
            <tr className="flex flex-col gap-2 items-center justify-center text-center mt-5">
              <td className="flex flex-col gap-2 items-center justify-center text-center">
                ...
                />
                <p className="text-gray-500 text-lg">No existe ese producto</p>
              </td>
            </tr>
          )}
        </tbody>
